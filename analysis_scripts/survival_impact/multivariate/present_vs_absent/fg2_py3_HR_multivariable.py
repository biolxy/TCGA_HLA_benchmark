...
df = pd.read_csv(filepath_or_buffer="infor_table.txt",
                 header='infer', sep='\t', index_col=None)
outpath = os.path.join(os.getcwd(),'py3_present_multivariable')
#print outpath
...
data = open(r'test_present_vs_absent.HR.plot.txt','r')
data.readline()
for line in data:
	entry = line.split('\t')
