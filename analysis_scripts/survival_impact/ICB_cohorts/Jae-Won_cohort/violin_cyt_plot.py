...
df = pd.read_csv(filepath_or_buffer=r"GSE126044_TPM.txt", header='infer', sep='\t', index_col=None,low_memory=False)
#sns.violinplot(x="Cancer Stage", y="Immune cytolytic activity (CYT)",hue = 'B44', hue_order = ['present','absent'],data=df,fliersize=0)
#ax = plt.subplot(111)
ax=sns.violinplot(x="Group", y="Immune cytolytic activity (CYT)",data=df,fliersize=0)
...
#sns.swarmplot(x="Group", y="Immune cytolytic activity (CYT)", size=3.5,data=df,dodge=True, color=".25")
#conditions = ['early SKCM','advanced READ','early BRCA','advanced OV']
dcb_df = df[df['Group']=='DCB']
ndb_df = df[df['Group']=='NDB']
textall = []
...
mannwhitneyu_p1 = round(scips.mannwhitneyu(pos_record,neg_record, alternative='greater')[1],4)
