...
ax = plt.subplot(111)
data = open(r'GSE126044_TPM.txt','r')
first = data.readline().strip()
colindex = coldict(first)
#tag1 = 'High TMB'
#tag2 = 'Low TMB'
os_index = colindex['Progression-free survival']
event_index = colindex['PFS_event']
...
print('Positive group median PFS: ' + str(float(exp_median)))
...
print('negative group median PFS: ' + str(float(control_median)))
...
# logrank pvalue
p = results.p_value
text0 = 'Jae-Won Cho NSCLC cohort under anti-PD1 therapy' + '\n'
text1 = r'CYT-High group Median PFS: %0.1f'%exp_median
text2 = r'CYT-Low group Median PFS: %0.1f'%control_median
...
ax.text(0.538, 0.825, textstr, transform=ax.transAxes, fontsize=13,horizontalalignment='center', verticalalignment='top', bbox=props)
ax.set_ylim(0,1.0)
ax.set_xlim(0,18)
#ax.set_ylabel('Probability of Overall survival  ', fontsize = 16)
ax.set_ylabel('Probability of progression-free survival  ', fontsize = 16)
ax.set_xlabel('Month',fontsize=16)
plt.savefig("Jae-Won_pfs.pdf", dpi=330)
plt.savefig("Jae-Won_pfs.png", dpi=330)
#plt.show()