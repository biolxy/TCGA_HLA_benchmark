...
ax = plt.subplot(111)
data = open(r'v2.GSE135222_infor.txt','r')
first = data.readline().strip()
colindex = coldict(first)
#tag1 = 'High TMB'
#tag2 = 'Low TMB'
os_index = colindex['PFS']
...
ax.text(0.618, 0.8, textstr, transform=ax.transAxes, fontsize=13, verticalalignment='top', bbox=props)
ax.set_ylim(0,1.0)
ax.set_xlim(0,21)
#ax.set_ylabel('Percent progression-free')
ax.set_ylabel('Probability of progression-free survival  ', fontsize = 16)
ax.set_xlabel('Month',fontsize=16)
plt.savefig("Jeong-Yeon-pfs.pdf", dpi=330)
plt.savefig("Jeong-Yeon-pfs.png", dpi=330)
#plt.show()