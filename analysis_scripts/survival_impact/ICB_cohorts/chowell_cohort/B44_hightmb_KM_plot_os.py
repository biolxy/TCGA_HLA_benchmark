...
cyt_index = colindex['group']

tag1 = 'B44.present&HighTMB'
tag2 = 'B44.absent&LowTMB'
...
ax = kmf_exp.fit(np.array(T_high), label='B44.present&HighTMB').plot(ax=ax,ci_show=True,linewidth=3)
...
ax = kmf_control.fit(np.array(T_low), label='B44.absent&LowTMB').plot(ax=ax,ci_show=True,linewidth=3)
...
text0_1 = 'B44.present&HighTMB vs B44.absent&LowTMB'
...
text3 = r'logRank p value = %.5f'%p
#text3 = r'$logRank\hspace{0.7}p\hspace{0.4}value=%.4f$'%p
textstr = text0 + '\n' +text0_1 + '\n' + text0_2 + '\n'+ text1 + '\n' + text3
#textstr = r'$logRank\hspace{0.7}p\hspace{0.4}value=3.9E-21$'
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
ax.text(0.538, 0.8, textstr, transform=ax.transAxes, fontsize=13,horizontalalignment='center', verticalalignment='top', bbox=props)
ax.set_ylim(0,1.0)
ax.set_xlim(0,95)
#ax.set_ylabel('Probability of OS(Months)  ', fontsize = 16)
ax.set_ylabel('Probability of overall survival  ', fontsize = 18)
ax.set_xlabel('Month',fontsize=18)
plt.savefig("Chowell_CTLA4_b44_highlowTMB.pdf", dpi=330)
plt.savefig("Chowell_CTLA4_b44_highlowTMB.png", dpi=330)
#plt.show()